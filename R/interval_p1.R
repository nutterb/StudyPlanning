...
#' @param p0 numeric, the hypothesize proportion. Must be on the interval (0, 1)
...
#'
#' @details Exactly one of \code{n}, \code{p0}, \code{E}, and 
...
#'  \item{\code{p0 }} Hypothesized proportion
...
#'  \item{\code{np }} The product of \code{n} and \code{p0}. 
#'  \item{\code{nq }} The product of \code{n} and \code{1 - p0}.
#' }
#' 
...
#'    the other arguments.
#'  \item When \code{p0 = NULL}, return the value of \code{p0} that 
...
#' interval_p1(p0 = 0.72, E = 0.01, alpha = 0.05)
#'
#' @export
                        ...
                        interval_max = NULL, ...)
{
  ...
  massert(~ p0 + E + interval_min + interval_max,
  ...
  if (sum(which_null) != 1)
  {
    coll$push("Exactly one of `n`, `p0`, `E`, and `alpha` must be NULL")
  }
  ...
  plan_fn <- function()
  {
    E - qnorm(alpha / ((tail == "both") + 1),
              lower.tail = FALSE) * sqrt((p0 * (1 - p0)) / n_est)
  }
  ...
  .params[["np"]] <- with(.params, n * p0)
  .params[["nq"]] <- with(.params, n * (1 - p0))
